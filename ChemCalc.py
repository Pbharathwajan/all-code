...
while True:
    ...
    if event == "Compounds":
        compoundlayout = [
            ...
            [gui.Text('Enter the Compound (Formula)'), gui.InputText(key = "molecule")],
            [gui.Button('Molecular mass'),gui.Button('Occurences'), gui.Button('Composition'), gui.Button('Stoichiometry'), gui.Button('cancel')]
        ...
        while True:
                ...
                if event2 == 'Composition':

                    compositionlayout=[
                        [gui.Text('Composition')],
                        [gui.Multiline(disabled=True,key="multi")],
                        [gui.Button('Ok'), gui.Button('Cancel')],
                     ]
                    win.Hide()
                    win21 = gui.Window("Composition", layout = compositionlayout, size =(300,300))

                    while True:
                        event21, values21 = win21.read(timeout = 10000)

                        if event21 == "Ok":
                            try:
                                for i in c.Compound(values2['molecule']).occurences.keys():
                                    win21.read()
                                    win21['multi'].print('Composition of element', i, 'is: ', c.Compound(values2['molecule']).percentage_by_mass(i), "\n")
                                #     win21['multi'].print(c.Compound(values2['molecule']).percentage_by_mass(i))
                                #    win21.Element('multi').update()
                ...
                if event2 == 'Stoichiometry':
                    ...
                    while True:
                        ...
                        if valuesstoich['grm'] == True:
                            try:
                                stoich=c.Compound(values2['molecule']).get_amounts(grams= amt)
                            except IndexError:
                                gui.popup(print('Invalid Input'))
                        elif valuesstoich['mol'] == True:
                            try:
                                stoich=c.Compound(values2['molecule']).get_amounts(moles= amt)
                            except IndexError:
                                gui.popup(print('Invalid Input'))
                        elif valuesstoich['mole'] == True:
                            try:
                                stoich=c.Compound(values2['molecule']).get_amounts(molecules= amt)
                            except IndexError:
                                gui.popup(print('Invalid Input'))
    ...
    if event == 'Instructions':
        instructions = '''Instructions for ChemCalc.py:
...
There maybe some unexpected errors with use of the code, in case of such events please restart the program again.
    ...
    win.Hide()
    win69 = gui.Window("Instructions", layout=instructionslay, size = (500,1000))