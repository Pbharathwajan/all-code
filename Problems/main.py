...
while True:
    ...
    if event == "Compounds":
        compoundlayout = [ 
            ...
            [gui.Button('Molecular mass'),gui.Button('Occurences'), gui.Button('Composition'), gui.Button('cancel')]
        ...
        while True:
                ...
                if event2 == 'Molecular mass':
                    A=c.compound(values2['molecule'])

                    mole_mass=A.molar_mass()
                    try:
                        gui.Popup('The molecular mass =', mole_mass, keep_on_top=True)
                        
                    except AttributeError and IndexError:
                        gui.popup('Invalid formula. Remember, It is "CaSe sEnSiTiVe"',keep_on_top=True)
                ...
                if event2 == 'Occurences':
                    ...
                    try:
                        ...
                        gui.popup('Invalid formula.')
                    
                    print (Occ.keys())
                if event2 == 'Composition':

                    try:
                        gui.popup('The composition of Element 1 is:', c.compound(values2['molecule']).percentage_by_mass.Occ.Keys())
                    except AttributeError:
